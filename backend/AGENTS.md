...
25. **DynamicLoopControllerMiddleware** - *(optional, if `dynamic_loop.enabled`, default on)* Run-level stagnation and wrap-up coordinator for long tasks. It observes completed ToolMessages before the next model call, queues concise change-strategy / wrap-up / non-interactive guidance, and can force finalization after repeated no-progress rounds by clearing structured and raw provider tool-call metadata on the latest AIMessage. It does not replace `recursion_limit`, token budgets, ToolProgressMiddleware, or LoopDetectionMiddleware.
26. **LoopDetectionMiddleware** - *(optional, if `loop_detection.enabled`)* Detects repeated tool-call loops; hard-stop clears both structured `tool_calls` and raw provider tool-call metadata before forcing a final text answer
